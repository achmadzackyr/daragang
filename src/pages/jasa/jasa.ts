...
@Component({
  selector: 'page-jasa',
  templateUrl: 'jasa.html',
})
export class JasaPage {
          ...
          popover.onDidDismiss((data,kab) => {
              if (data!=null) {
                ...
                if(this.filteredItems.ciamis) {
                  this.filterCtrl.cariBerdasarCiamisJasa(this.filteredItems.ciamis,this.jasa$);
                  this.jasa$ = this.filterCtrl.jasa$;
                } else if(this.filteredItems.banjar) {
                  this.filterCtrl.cariBerdasarBanjarJasa(this.filteredItems.banjar,this.jasa$);
                  this.jasa$ = this.filterCtrl.jasa$;
                } else if(this.filteredItems.pangandaran) {
                  this.filterCtrl.cariBerdasarPangandaranJasa(this.filteredItems.pangandaran,this.jasa$);
                  this.jasa$ = this.filterCtrl.jasa$;
                } else if(this.filteredItems.kotatasikmalaya) {
                  this.filterCtrl.cariBerdasarKotatasikmalayaJasa(this.filteredItems.kotatasikmalaya,this.jasa$);
                  this.jasa$ = this.filterCtrl.jasa$;
                } else if(this.filteredItems.kabtasikmalaya) {
                  this.filterCtrl.cariBerdasarKabtasikmalayaJasa(this.filteredItems.kabtasikmalaya,this.jasa$);
                  this.jasa$ = this.filterCtrl.jasa$;
                } else if(this.filteredItems.garut) {
                  this.filterCtrl.cariBerdasarGarutJasa(this.filteredItems.garut,this.jasa$);
                  this.jasa$ = this.filterCtrl.jasa$;
                } else if(this.filteredItems.sumedang) {
                  this.filterCtrl.cariBerdasarSumedangJasa(this.filteredItems.sumedang,this.jasa$);
                  this.jasa$ = this.filterCtrl.jasa$;
                }
                else if(kab!="backdrop") {
                  this.filterCtrl.cariBerdasarKabupatenCODJasa(kab,this.jasa$);
                  this.jasa$ = this.filterCtrl.jasa$;
                }
 ...
 getItems(searchbar) {
    ...
    if(searchbar.srcElement.value==""&&this.jasa$==undefined) {
      this.kategori(null);
      this.focusOut();
    ...
    } else if(this.jasa$==undefined) {
      alert("Pilih Dulu Kategori Yang Akan Dicari");
      return;
    } else {
      this.filterCtrl.getItemsJasa(searchbar,this.jasa$);
      this.jasa$ = this.filterCtrl.jasa$;  
      this.focusOut();
      this.content.scrollToBottom(500);
    }
   ...
   this.pilih=true;
    ...
    snapshot.forEach(function(childSnapshot) {
      ...
      var queryDagangan;
            ...
            if(kat==null) {
              queryDagangan = firebase.database().ref("jasa").child(kunci).orderByChild('order');
            } else {
              queryDagangan = firebase.database().ref("jasa").child(kunci).orderByChild("kategori").equalTo(kat);
            }
    ...
    this.jasa$ = arrayDaganganData;
    this.jasaList = arrayDaganganData;
    this.kat = kat;
    if (this.kat==null) {
      this.kat="Semua Jenis Jasa";
    }
  }).then((x) => {
  ...
  ionViewDidLoad() {
    ...
    if(this.pilih) {
      this.kategori(this.kat).then(()=> {
        this.filterCtrl.jasa$ = this.jasa$;
        this.filterCtrl.jasaList = this.jasaList;
      });
    }
  }
      ...
      return;
        ...
        if (userData){
          ...
          if(this.likerUid!=item.keyUser) {
            ...
            this.notifikasi.jenisDagangan = "jasa";
